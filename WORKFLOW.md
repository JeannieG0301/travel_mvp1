...
> 本文档定义**项目经理、产品经理、UI/UX 设计师、架构师、Tech Lead、开发团队、测试团队**七个 Agent 的协作方式与职责边界。**角色定义详见 `ROLES.md`**。

---

## 一、七方角色（摘要）

> 完整职责、产出物、依赖见 `ROLES.md`。

| Agent | 职责 | 产出物 | 依赖 |
|-------|------|--------|------|
| **项目经理** | 协调流程、分配任务、跟踪状态、维护文档 | `WORKFLOW.md`、BACKLOG 状态、各 agent 的 task prompt | 无 |
| **产品经理** | 定义需求、范围、输出格式；拍板决策；不负责 UI 设计稿 | `MVP1_PRD.md`、`INPUT_PARAMS_SPEC.md`、`OUTPUT_FORMAT_SPEC.md`、`UI_REDESIGN_SPEC.md` | 无 |
| **UI/UX 设计师** | 界面设计、交互流程、视觉规范、可预览设计稿 | 设计说明；`frontend/preview/` 静态预览页 | 产品经理的 UI 需求文档 |
...
┌─────────────────────────────────────────────────────────────────────────────┐
│  1. 需求与规范（产品经理）                                                      │
│     PRD、INPUT_PARAMS_SPEC、OUTPUT_FORMAT_SPEC；UI 改版时产出 UI_REDESIGN_SPEC   │
└─────────────────────────────────┬───────────────────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  2. UI 设计（UI/UX 设计师）※ 仅在有 UI 改版需求时执行                             │
│     设计说明、frontend/preview 静态预览页                                        │
└─────────────────────────────────┬───────────────────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
- **工作内容**：
  ...
  - UI 改版时产出 `UI_REDESIGN_SPEC.md`（需求与验收标准）
- **输出**：规范文档。**不负责可预览的 UI 设计稿**，该工作由 UI/UX 设计师负责。
- **协作**：架构师、UI/UX 设计师、Tech Lead、开发、测试均以这些文档为准。

### 3.2 UI/UX 设计师（UI 设计）

- **触发**：产品经理产出 UI 需求文档（如 `UI_REDESIGN_SPEC.md`）后，开发实施前。**仅在有 UI 改版需求时执行**。
- **工作内容**：
  - 产出可预览的 UI 设计稿，含设计说明 + 静态 HTML 预览页或 UI 示意图
  - 界面设计、交互流程、视觉规范
...
- **触发**：产品规范确定后、架构变更时。若有 UI 任务，则需 UI/UX 设计稿确认后。
- **工作内容**：
...
- **工作内容**：
  - 阅读 `MVP1_PRD`、`INPUT_PARAMS_SPEC`、`OUTPUT_FORMAT_SPEC`、`ARCHITECTURE`；若有 UI 任务则参考 UI/UX 设计稿
...
| UI/UX 设计师产出设计稿 | `frontend/preview/` 或约定路径；设计说明与需求文档同步 |
...
| 上线部署 | #12 配置、DEPLOY.md、本地 Git 已就绪；待项目负责人：GitHub 建仓+push、Render 创建服务+配置 KEY，获取公网 URL 后汇报 |
...
2. **产品定规范**：PRD、输入、输出三份文档是唯一源头；UI 改版时产出 UI_REDESIGN_SPEC。
3. **UI/UX 出设计**：有 UI 任务时，UI/UX 设计师产出可预览设计稿，确认后再进入 Tech Lead。
4. **架构定骨架**：ARCHITECTURE + BACKLOG 初版，确保可扩展。
5. **Tech Lead 拆任务**：高优任务必拆子任务，验收标准写进 BACKLOG。
6. **开发按单实现**：按子任务顺序做，改规范必同步文档。
7. **测试对照验收**：以规范文档和 BACKLOG 验收标准为准。
...
- 协调七方协作顺序，起草并下发各 agent 的 task prompt
...
当 ROLES.md 或协作流程发生角色相关变更时：

1. **多 Agent 项目**：按 `docs/AGENT_ROLE_SYNC.md` 在各 Cursor Agent（产品、UI/UX、架构、Tech Lead、开发、测试）中同步（Instructions 或对话粘贴通知）。
2. **Task prompt**：下轮分配任务时，在 task prompt 开头附带：
   ```
   【角色分工已更新】七方协作，UI/UX 设计师已独立于产品经理。完整定义见 ROLES.md。
   ```
...
| 2026-02-23 | 开发团队完成本地 Git 配置（main 分支、.gitignore）、新增 PUSH_TO_GITHUB.md；待项目负责人创建 GitHub 仓库并 push |
| 2026-02-24 | 六方调整为七方：UI/UX 设计师独立于产品经理；角色定义以 ROLES.md 为准；协作流程增加 UI 设计阶段 |
| 2026-02-24 | 新增 docs/AGENT_ROLE_SYNC.md：多 Agent 角色同步指南；更新检查清单与 WORKFLOW 7.3 |
